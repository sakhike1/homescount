...
export default async function RentBrowseTeaserAsync({ browseHref, filters }: Props) {
  ...
  const preview = properties.slice(0, 6).map((p, i) => ({
    ...
    images: p.images.length
      ? p.images
      : [{ url: getPropertyImageUrl([], i) }],
  }))

  return (
    <RentBrowseTeaser properties={preview} browseHref={browseHref} isDemo={isDemo} />
  )
}
